...
- **Found and fixed a second real bug in the rate limiter: it keyed on the raw TCP peer address and ignored `X-Forwarded-For` entirely.** Tested against a real running container with two different `X-Forwarded-For` header values simulating two different clients behind a reverse proxy: both shared one rate-limit bucket, so one client exhausting the limit blocked every other client too. Added opt-in `ProxyFix` support gated by `PAPVISION_TRUST_PROXY_HOPS` (default `0`, i.e. off — trusting `X-Forwarded-For` with no real proxy in front would let any client self-report an arbitrary rate-limit identity and bypass the limit entirely, which is worse). Re-tested with `PAPVISION_TRUST_PROXY_HOPS=1` against the same two simulated clients: correctly separated into independent buckets. Both the default-off and opt-in-on behaviours are pinned by a unit test and documented in the README config table and "Known limitations".
